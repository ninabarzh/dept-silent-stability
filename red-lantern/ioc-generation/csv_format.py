...
class CSVThreatFeedGenerator:
            ...
            for asn in malicious_asns:
                writer.writerow(
                    [
                        ...
                        (
                            ";".join(asn.associated_prefixes)
                            if asn.associated_prefixes
                            else ""
                        ),
                        asn.confidence,
    ...
    @staticmethod
    def generate_prefix_feed(
        suspicious_prefixes: list[SuspiciousPrefix], filename: str
    ) -> None:
            ...
            for prefix in suspicious_prefixes:
                writer.writerow(
                    [
                        ...
                        prefix.rpki_status,
                        (
                            ",".join(prefix.announcement_path)
                            if hasattr(prefix, "announcement_path")
                            else ""
                        ),
                        prefix.confidence,
                    ]
    ...
    @staticmethod
    def generate_infrastructure_feed(
        infrastructure: list[AttackerInfrastructure], filename: str
    ) -> None:
            ...
            for infra in infrastructure:
                ...
                writer.writerow(
                    [
                        ...
                        (
                            len(infra.associated_attacks)
                            if infra.associated_attacks
                            else 0
                        ),
                        (
                            geolocation.get("country", "Unknown")
                            if isinstance(geolocation, dict)
                            else "Unknown"
                        ),
                        (
                            geolocation.get("asn", "Unknown")
                            if isinstance(geolocation, dict)
                            else "Unknown"
                        ),
                        infra.confidence,
    ...
    def generate_combined_feed(
        ...
        with open(filename, "w", newline="", encoding="utf-8") as file:
            ...
            for asn in malicious_asns:
                associated_prefixes = (
                    list(asn.associated_prefixes)
                    if hasattr(asn, "associated_prefixes")
                    else []
                )
                writer.writerow(
                    [
                        ...
                        asn.confidence,
                        (
                            f"Prefixes: {', '.join(associated_prefixes[:3])}"
                            if associated_prefixes
                            else "No associated prefixes"
                        ),
                        ";".join(asn.attack_types) if asn.attack_types else "",
                    ]
                )
            ...
            for infra in infrastructure:
                writer.writerow(
                    [
                        ...
                        (
                            f"attacks:{len(infra.associated_attacks)}"
                            if infra.associated_attacks
                            else "attacks:0"
                        ),
                    ]
...
def main():
    ...
    CSVThreatFeedGenerator.generate_prefix_feed(
        suspicious_prefixes, "suspicious_prefixes.csv"
    )
    CSVThreatFeedGenerator.generate_infrastructure_feed(
        infrastructure, "attacker_infrastructure.csv"
    )

...
Complete threat modelling system for BGP hijacking scenarios with Python 3.12
...
All imports are included and code is production-ready.
"""
...
class AttackTreeNode:
    ...
    def calculate_total_cost(self) -> float:
        ...
        if self.node_type == 'and':
            return self.cost + sum(child.calculate_total_cost() for child in self.children)
        elif self.node_type == 'or':
            return self.cost + min(child.calculate_total_cost() for child in self.children)
        else:
            return self.cost
...
class ScenarioThreatModelExtractor:
    ...
    @staticmethod
    def extract_playbook1_scenario(log_lines: list[str]) -> ThreatScenario:
        """
        ...
        events.append(ThreatEvent(
            timestamp=ScenarioThreatModelExtractor.parse_timestamp("Jan 01 00:00:00"),
        ...
        events.append(ThreatEvent(
            timestamp=ScenarioThreatModelExtractor.parse_timestamp("Jan 01 00:01:05"),
    ...
    @staticmethod
    def extract_playbook2_scenario(log_lines: list[str]) -> ThreatScenario:
        """
        ...
        events.append(ThreatEvent(
            timestamp=ScenarioThreatModelExtractor.parse_timestamp("Jan 01 00:01:00"),
        ...
        events.append(ThreatEvent(
            timestamp=ScenarioThreatModelExtractor.parse_timestamp("Jan 01 00:02:00"),
            phase=AttackPhase.DEFENCE_EVASION,
            ...
            data_sources=["RPKI repository logs", "ROA creation audit logs", "RIR portal logs"],
            mitigations=[
        ...
        events.append(ThreatEvent(
            timestamp=ScenarioThreatModelExtractor.parse_timestamp("Jan 01 00:40:00"),
            ...
            data_sources=["RPKI repository monitoring", "ROA publication feeds", "Validator sync logs"],
            mitigations=[
        ...
        events.append(ThreatEvent(
            timestamp=ScenarioThreatModelExtractor.parse_timestamp("Jan 01 00:45:00"),
            phase=AttackPhase.IMPACT,
            ...
            data_sources=["Validator logs", "RPKI validation status monitoring", "Validator consensus tracking"],
            mitigations=[
    ...
    @staticmethod
    def extract_playbook3_scenario(log_lines: list[str]) -> ThreatScenario:
        """
        ...
        events.append(ThreatEvent(
            ...
            data_sources=["BGP monitoring", "BMP feeds", "Route announcements", "Route collectors"],
            mitigations=[
        ...
        # Event 2: Defence Evasion
        events.append(ThreatEvent(
            timestamp=ScenarioThreatModelExtractor.parse_timestamp("Jan 01 00:03:00"),
            phase=AttackPhase.DEFENCE_EVASION,
            ...
            data_sources=["RPKI validation logs", "Validator decision logs", "ROA evaluation traces"],
            mitigations=[
        ...
        events.append(ThreatEvent(
            timestamp=ScenarioThreatModelExtractor.parse_timestamp("Jan 01 00:07:00"),
            phase=AttackPhase.IMPACT,
            ...
            data_sources=["Flow data", "Traffic analysis", "BGP best path selection", "NetFlow/IPFIX"],
            mitigations=[
        ...
        # Event 4: Persistence
        events.append(ThreatEvent(
            timestamp=ScenarioThreatModelExtractor.parse_timestamp("Jan 01 01:32:00"),
            ...
            data_sources=["BGP update monitoring", "Route history", "Uptime tracking", "Session logs"],
            mitigations=[
...
class MITREATTACKMapper:
    ...
    TECHNIQUE_DATABASE: ClassVar[dict[str, ATTACKTechnique]] = {
        ...
        'T1565.002': ATTACKTechnique(
            ...
            detection_methods=[
                ...
                'Analyze traffic patterns for redirection',
    ...
    def map_scenario_to_attack(scenario: ThreatScenario) -> list[ATTACKTechnique]:
        ...
        phase_technique_mapping = {
            ...
            (AttackPhase.DEFENCE_EVASION, "Use of Legitimate Infrastructure"): 'T1562.001',
            (AttackPhase.DEFENCE_EVASION, "Subvert Trust Controls"): 'T1562.001',
            (AttackPhase.DEFENCE_EVASION, "Exploit Public-Facing Application"): 'T1562.001',
    ...
    def generate_attack_navigator_layer(
            ...
            layer_name: str = "BGP Hijacking Scenarios"
    ) -> dict:
        ...
        for scenario in scenarios:
            ...
            for technique in mapped_techniques:
                ...
                if scenario.scenario_name not in techniques_coverage[technique.technique_id]['scenarios']:
                    techniques_coverage[technique.technique_id]['scenarios'].append(scenario.scenario_name)
...
class AttackTreeGenerator:
    ...
    def build_playbook2_attack_tree() -> AttackTreeNode:
        ...
        obtain_creds = AttackTreeNode(
            ...
            cost=2.0,
            detectability=0.3,
            mitigations=["Strong password policies", "MFA enforcement", "Credential monitoring"],
            detection_methods=["Failed login attempts", "Credential stuffing detection", "Dark web monitoring"]
        )
        ...
        phishing = AttackTreeNode(
            ...
            cost=1.0,
            detectability=0.5,
            mitigations=["Security awareness training", "Email filtering", "Anti-phishing tools"],
            detection_methods=["Email security gateways", "User reporting", "Suspicious link detection"]
        )
        ...
        credential_stuffing = AttackTreeNode(
            ...
            cost=0.5,
            detectability=0.7,
            mitigations=["Password reuse policies", "Breach monitoring", "Account lockout policies"],
            detection_methods=["Failed login monitoring", "Impossible travel", "Anomalous auth patterns"]
        )
        ...
        insider_threat = AttackTreeNode(
            ...
            cost=5.0,
            detectability=0.2,
            mitigations=["Background checks", "Insider threat program", "Behavioral analytics"],
            detection_methods=["User behavior analytics", "Privilege escalation monitoring"]
        )
        ...
        anonymized_access = AttackTreeNode(
            ...
            cost=0.5,
            detectability=0.8,
            mitigations=["Block Tor exit nodes", "Geofencing", "Risk-based authentication"],
            detection_methods=["Tor exit node detection", "VPN/proxy detection", "Geographic anomalies"]
        )
        ...
        submit_roa = AttackTreeNode(
            ...
            cost=0.5,
            detectability=0.7,
            mitigations=["Out-of-band verification", "Secondary approval", "ROA change monitoring"],
            detection_methods=["Anomalous ROA requests", "Prefix ownership validation", "Request source analysis"]
        )
        ...
        roa_publication = AttackTreeNode(
            ...
            cost=0.0,
            detectability=0.6,
            mitigations=["Manual review process", "Automated validation", "Community review period"],
            detection_methods=["ROA publication monitoring", "Anomaly detection on new ROAs"]
        )
        ...
        validator_sync = AttackTreeNode(
            ...
            cost=0.0,
            detectability=0.5,
            mitigations=["Multi-source validation", "Historical comparison", "Community alerting"],
            detection_methods=["Validator consensus monitoring", "ROA change detection", "Validation status tracking"]
        )
        ...
        find_vuln = AttackTreeNode(
            ...
            cost=5.0,
            detectability=0.3,
            mitigations=["Regular security audits", "Penetration testing", "Bug bounty program"],
            detection_methods=["Intrusion detection", "Unusual API calls", "System log analysis"]
        )
        ...
        exploit_vuln = AttackTreeNode(
            ...
            detection_methods=["IDS/IPS alerts", "Anomalous database writes", "Audit log analysis"]
        )
    ...
    def build_playbook3_attack_tree() -> AttackTreeNode:
        ...
        own_asn = AttackTreeNode(
            ...
            detection_methods=["ASN reputation monitoring", "Historical behavior analysis"]
        )
        ...
        compromise_router = AttackTreeNode(
            ...
            detection_methods=["Configuration change detection", "Unauthorized access alerts"]
        )
        ...
        announce_prefix = AttackTreeNode(
            ...
            cost=0.5,
            detectability=0.8,
            mitigations=["Prefix filtering", "Anomaly detection", "Alert on more-specifics"],
            detection_methods=["BGP monitoring", "Route announcement detection", "BMP feeds"]
        )
        ...
        intercept_traffic = AttackTreeNode(
            ...
            detection_methods=["Traffic pattern analysis", "Latency monitoring", "Geographic anomalies"]
        )
    ...
    def generate_attack_tree_dot(tree: AttackTreeNode) -> str:
        ...
        def add_node(node: AttackTreeNode, parent_id: Optional[str] = None) -> str:
    ...
    @staticmethod
    def analyze_attack_paths(tree: AttackTreeNode) -> dict:
        """Analyze all possible attack paths through the tree."""
        paths = []
        ...
        def find_paths(node: AttackTreeNode, current_path: list[str]) -> None:
            ...
            if not node.children:
                ...
                paths.append({
                    ...
                    'total_cost': sum(n.cost for n in _get_nodes_in_path(tree, current_path)),
                    'min_detectability': min((n.detectability for n in _get_nodes_in_path(tree, current_path)),
                                             default=0.0)
                })
                return
        ...
        def _get_nodes_in_path(root: AttackTreeNode, path_names: list[str]) -> list[AttackTreeNode]:
...
class DetectionGapAnalyser:
    """Analyzes detection coverage and identifies gaps."""
    ...
    def analyze_scenario_coverage(self, scenario: ThreatScenario) -> list[DetectionGap]:
        """Analyze detection coverage for a scenario and identify gaps."""
        gaps = []
        ...
        for event in scenario.events:
            ...
            if missing_sources:
                gaps.append(DetectionGap(
                    ...
                    severity=self._calculate_severity(event.phase, len(missing_sources)),
            ...
            if not has_detection and self.available_data_sources.intersection(required_sources):
                gaps.append(DetectionGap(
                    ...
                    required_data_sources=list(required_sources.intersection(self.available_data_sources))
                ))
            ...
            if not missing_sources and not has_detection:
                gaps.append(DetectionGap(
                    ...
                    technique_name=event.technique,
                    phase=event.phase.value,
                    severity='critical' if event.phase in {AttackPhase.INITIAL_ACCESS, AttackPhase.IMPACT} else 'high',
                    ...
                    recommended_actions=[
                        ...
                        f"Create detection rules immediately",
    ...
    def generate_coverage_report(self, scenarios: list[ThreatScenario]) -> dict:
        ...
        for scenario in scenarios:
            scenario_gaps = self.analyze_scenario_coverage(scenario)
            ...
            for event in scenario.events:
                ...
                # Check coverage
                available = len(set(event.data_sources).intersection(self.available_data_sources))
                technique_coverage[tech_key]['data_sources_available'] = max(
                    technique_coverage[tech_key]['data_sources_available'],
                    available
                )
        ...
        for tech, cov in technique_coverage.items():
            if cov['data_sources_available'] == cov['data_sources_required'] and cov['has_detection_rule']:
        ...
        return {
            ...
            'coverage_percentage': (coverage_stats['covered_techniques'] / coverage_stats['total_techniques'] * 100)
            if coverage_stats['total_techniques'] > 0 else 0.0
        }
    ...
    def generate_gap_report(gaps: list[DetectionGap]) -> str:
        ...
        for severity in ['critical', 'high', 'medium', 'low']:
            ...
            report.append(f"\n{'─' * 80}")
            report.append(f"{severity.upper()} SEVERITY GAPS ({len(by_severity[severity])})")
            report.append('─' * 80)
            ...
            for i, gap in enumerate(by_severity[severity], 1):
                ...
                report.append(f"    Affected Scenarios: {', '.join(gap.affected_scenarios)}")
                report.append(f"    Required Data Sources:")
...
def main():
    ...
    playbook2_analysis = tree_gen.analyze_attack_paths(playbook2_tree)
    print(f"\nPlaybook 2: {playbook2_analysis['total_paths']} possible attack paths")
    if playbook2_analysis['easiest_path']:
        print(f"Easiest path cost: {playbook2_analysis['easiest_path']['total_cost']:.1f}")

    # Detection Gap Analysis
    print("\n6. Analyzing Detection Gaps...")
    ...
    with open('threat_model_report.json', 'w') as f:
        json.dump({
            ...
            'attack_tree_analysis': {
                ...
                'playbook3': tree_gen.analyze_attack_paths(playbook3_tree)
            }
        }, f, indent=2)
